...
inject_package_resource() {
  ...
  cp "${file}" "${work}/Resources/"
  chmod +x "${work}/Resources/$(basename "${file}")"
...
# Applications that must not be running while P(X3) is installed or removed.
#
# A host that has the plug-in loaded holds the bundle open, so replacing it
# underneath leaves the host running stale code and can leave a half-written
# bundle behind. The uninstaller has the same problem in reverse.
#
# The two arrays are index-aligned and are the single source for both the shell
# check and the Installer's JavaScript, so the names and the exit codes cannot
# drift apart. Patterns are matched against the process name only, anchored, so
# "Live" does not match "LiveWhatever".
PX3_HOST_NAMES=(
  "Logic Pro" "GarageBand" "MainStage" "Ableton Live" "Pro Tools"
  "Cubase" "Nuendo" "Studio One" "REAPER" "Bitwig Studio"
  "FL Studio" "Digital Performer" "Reason" "Ardour" "Waveform"
  "LUNA" "Renoise" "MASCHINE" "Audio Plugin Host" "the P(X3) standalone app"
)
PX3_HOST_PATTERNS=(
  "Logic Pro( X)?" "GarageBand" "MainStage" "Live" "Pro Tools"
  "Cubase.*" "Nuendo.*" "Studio One.*" "REAPER|reaper" "Bitwig Studio"
  "FL Studio.*" "Digital Performer.*" "Reason.*" "Ardour.*" "Waveform.*"
  "LUNA" "Renoise" "MASCHINE.*|Maschine.*" "AudioPluginHost|pluginval|auval" "PX3 Synth"
)

# Writes the host-detection script used by both packages. It exits 0 when
# nothing is running, or with the 1-based index of the first host it finds.
write_host_check_script() {
  local dest="$1"
  {
    cat <<'HDR'
#!/bin/sh
# Generated by scripts/build-release.sh
# Exit 0: no audio host running. Exit N: PX3_HOST_NAMES[N-1] is running.
running=$(ps -Ac -o comm= 2>/dev/null)
index=0
check() {
  index=$((index + 1))
  if printf '%s\n' "${running}" | grep -qE "^($1)$"; then
    exit ${index}
  fi
}
HDR
    local pattern
    for pattern in "${PX3_HOST_PATTERNS[@]}"; do
      printf 'check %s\n' "'${pattern}'"
    done
    printf 'exit 0\n'
  } > "${dest}"
...
# Emits the <script> block and <installation-check> element that turn a non-zero
# exit from the check above into a readable "quit X first" dialog, rather than
# the Installer's generic failure message.
host_check_distribution_xml() {
  local verb="$1"     # "installing" or "removing"
  local js_names=""
  local name
  for name in "${PX3_HOST_NAMES[@]}"; do
    [[ -n "${js_names}" ]] && js_names+=", "
    js_names+="'${name//\'/\\\'}'"
  done
  ...
  cat <<EOF
  <script><![CDATA[
function px3HostCheck()
{
    var names = [${js_names}];
    var code = 0;
    try {
        code = system.run('check-hosts');
    } catch (e) {
        // If the check itself cannot run, do not block the user.
        return true;
    }
    if (code == 0) {
        return true;
    }
    var app = (code >= 1 && code <= names.length) ? names[code - 1] : 'an audio application';
    my.result.title = 'Quit ' + app + ' first';
    my.result.message = app + ' is currently running. Quit it, and any other DAW '
        + 'or audio host, before ${verb} P(X3).\n\n'
        + 'A host with the plug-in loaded keeps the old version in memory and can '
        + 'leave the plug-in in a broken state.';
    my.result.type = 'Fatal';
    return false;
}
  ]]></script>
  <installation-check script="px3HostCheck()"/>
EOF
}
...
write_host_check_script "${PKG_SHARED_SCRIPTS_DIR}/px3-check-hosts"
cat > "${PKG_SHARED_SCRIPTS_DIR}/preinstall" <<'PREEOF'
#!/bin/sh
# Refuses to install while an audio host is open: a host with the plug-in
# loaded holds the bundle open, and replacing it underneath leaves the host
# running stale code.
"$(dirname "$0")/px3-check-hosts"
status=$?
if [ ${status} -ne 0 ]; then
  echo "P(X3): an audio application is still running; quit it and try again." >&2
  exit 1
fi
exit 0
PREEOF
chmod +x "${PKG_SHARED_SCRIPTS_DIR}/preinstall"
...
write_host_check_script "${PRODUCT_RESOURCES_DIR}/check-hosts"
INSTALLER_HOST_CHECK_XML="$(host_check_distribution_xml "installing")"
...
inject_package_resource "${PKG_PATH}" "${PRODUCT_RESOURCES_DIR}/check-hosts"
...
grep -F "installation-check" "${PKG_EXPANDED_DIR}/Distribution" >/dev/null 2>&1 \
  || die "Final installer does not run the running-host check"

[[ -x "${PKG_EXPANDED_DIR}/Resources/check-hosts" ]] \
  || die "Final installer is missing the running-host check helper (productbuild dropped it)"
...
if [[ "${BUILD_UNINSTALLER}" == true ]]; then
  ...
  write_host_check_script "${UNINSTALLER_RESOURCES_DIR}/check-hosts"
  UNINSTALLER_HOST_CHECK_XML="$(host_check_distribution_xml "removing")"
...
EOF
  ...
  # the Installer's JavaScript context; this one is a plain script and will stop
  # the package regardless of what that context supports.
  write_host_check_script "${UNINSTALLER_SCRIPTS_DIR}/px3-check-hosts"
  cat > "${UNINSTALLER_SCRIPTS_DIR}/preinstall" <<'PREEOF'
#!/bin/sh
# Refuses to run while an audio host is open. See px3-check-hosts.
"$(dirname "$0")/px3-check-hosts"
status=$?
if [ ${status} -ne 0 ]; then
  echo "P(X3): an audio application is still running; quit it and try again." >&2
...
exit 0
PREEOF
  chmod +x "${UNINSTALLER_SCRIPTS_DIR}/preinstall"
...
EOF
  ...
  inject_package_resource "${UNINSTALLER_PKG_PATH}" "${UNINSTALLER_RESOURCES_DIR}/check-hosts"
  ...
  [[ -x "${UNINSTALLER_EXPANDED_DIR}/Resources/check-hosts" ]] \
    || die "Uninstaller is missing the running-host check helper"
